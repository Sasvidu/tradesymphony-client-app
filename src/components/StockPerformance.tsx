...
export function StockPerformance() {
  ...
  useEffect(() => {
    ...
    // Get data for AAPL for the last 30 days
    const startDate = new Date();
    startDate.setDate(startDate.getDate() - 30);
    
    axios
      .get(`/api/stocks?symbol=AAPL&startDate=${startDate.toISOString()}`)
      .then((response) => {
        ...
        <Flex>
          <div>
            ...
            <Text className="text-gray-400">
              {formatDate(stockData.startDate)} - {formatDate(stockData.endDate)}
            </Text>
          </div>
